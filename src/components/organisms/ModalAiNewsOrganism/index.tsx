  ...
  return <div className={'modal-ai-news-organism'}>
    ...
    <div className={'ai-news-content'}>
      ...
      <CommonModal
        ...
        footer={[
          <CommonButton isOutline={true} content={'キャンセル'} handleClick={handleOpenModal} />,
          <CommonButton content={"登録"} handleClick={handleOpenModal}/>
        ]}
      >
        <div style={{ width: '616px' }} className={' '}>
          <InputWithTitle name={'title'} placeHolder={''} inputRef={titleRef} title={'タイトル'} type={InputType.TEXT} />
        </div>
        <div style={{ width: '616px' }} className={'mt-6'}>
          <InputWithTitle type={InputType.AREA} name={'overview'} placeHolder={''} inputRef={overviewRef}
                          title={'概要'} />
        </div>
        <div style={{ width: '217px' }} className={'mt-6'}>
          <InputWithTitle type={InputType.DATE} name={'postingDateAndTime'} placeHolder={''}
                          inputRef={postingDateAndTimeRef} title={'元記事投稿日時'} />
        </div>
        <div style={{ width: '616px' }} className={'mt-6'}>
          <InputWithTitle name={'link'} placeHolder={''} inputRef={linkRef} type={InputType.TEXT} title={'リンク'} />
        </div>
        <div style={{ width: '368px' }} className={'mt-6'}>
          <InputWithTitle options={distinguishs} name={'tag'} inputRef={distinguishRef} type={InputType.SELECT}
                          title={'区分'} />
        </div>
        <div className={'mt-6'}>
          <TagSelect tags={tags} title={'タグ'} inputRef={tagRef} />
        </div>
